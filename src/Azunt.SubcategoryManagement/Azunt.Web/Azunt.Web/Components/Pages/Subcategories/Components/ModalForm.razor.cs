...
namespace Azunt.Web.Components.Pages.Subcategories.Components;
...
public partial class ModalForm : ComponentBase
{
    ...
    [Parameter] public Subcategory ModelSender { get; set; } = null!;

    public Subcategory ModelEdit { get; set; } = null!;
    ...
    [Inject]
    public ISubcategoryRepository RepositoryReference { get; set; } = null!;

    [Inject]
    private ISubcategoryStorageService SubcategoryStorage { get; set; } = null!;
    ...
    protected async Task HandleSubcategoryChange(InputFileChangeEventArgs e)
    {
        selectedFile = e.File;

        using var stream = selectedFile.OpenReadStream(maxAllowedSize: 10 * 1024 * 1024);
        var subcategoryUrl = await SubcategoryStorage.UploadAsync(stream, selectedFile.Name);

        // 파일명 저장
        ModelEdit.FileName = Path.GetFileName(subcategoryUrl);
    ...
    protected async Task HandleValidSubmit()
    {
        ...
        bool isSubcategoryReplaced = ModelSender.Id > 0 &&
                              !string.IsNullOrWhiteSpace(ModelSender.FileName) &&
                              ModelSender.FileName != ModelEdit.FileName;

        if (isSubcategoryReplaced)
        {
            // 기존 파일 삭제
            await SubcategoryStorage.DeleteAsync(ModelSender.FileName!);
        }
